...
###############################################################################
# THIS MODEL IS DESIGNED TO RUN FINETUNING ON VIT Model 
# Downloaded from Huggingface
###############################################################################
def build_vit():
    model = TFViTForImageClassification.from_pretrained("google/vit-base-patch16-224-in21k", num_labels=num_classes)
    return model
...
if model_name == "EfficientNetV2":
    ...
    checkpoint_h5 = ModelCheckpoint(
     "gs://spotted-models-deployment/matching_models/embedding_model/tf_model.h5",
    monitor='val_loss',
    verbose=1,
    save_best_only=True,
    mode='min',
...
if model_name == "EfficientNetV2_finetuning":
    ...
    wandb.init(
          project="vit-serverless-training",
          config={
                  "learning_rate": learning_rate,
                  "epochs": epochs,
                  "batch_size": batch_size,
...
training_results = model.fit(
    ...
    epochs=epochs,
    callbacks=[WandbCallback(), checkpoint_h5],
    verbose=1)
